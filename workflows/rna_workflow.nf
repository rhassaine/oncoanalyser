...
*/
// Parse input samplesheet
// NOTE: This is done early and outside of gpars so that we can access synchronously and prior to pipeline execution
inputs = Utils.parseInput(params.input, workflow.stubRun, log)
...
workflow RNA_WORKFLOW {
    // Create a channel for software versions
    ch_versions = Channel.empty()
    ...
    // Set GRIDSS config
    gridss_config = params.gridss_config ? file(params.gridss_config) : hmf_data.gridss_config
