...
process SAGE_SOMATIC {
    ...
    # Copy REDUX BQR files (TSV and PNG) to SAGE output directory
    # ORANGE expects BQR PNG plots in sage_dir for generating quality control reports
    # SAGE expects BQR TSV files for jitter analysis
    # These files are generated by REDUX: *.redux.bqr.tsv and *.redux.bqr.png
    for bqr_file in *.redux.bqr.{tsv,png}; do
        if [[ -f "\${bqr_file}" ]]; then
            cp "\${bqr_file}" somatic/
        fi
    done
