...
process SAGE_SOMATIC {
    ...
    # Copy REDUX BQR files to SAGE output directory
    # ORANGE expects BQR files in sage_dir for generating quality control plots
    # These files are generated by REDUX and consumed by SAGE via -jitter_bqr_dir
    for bqr_file in *.redux.bqr.tsv; do
        if [[ -f "\${bqr_file}" ]]; then
            cp "\${bqr_file}" somatic/
        fi
    done
